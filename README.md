...
4. 读取“奶量”滑动选择器的当前值，逐步滑动到 `150 ml`。
5. 停在美柚确认页面，由用户检查并手动保存。

首次使用必须在系统无障碍设置中手动开启“美柚瓶喂记录自动化”。服务仅监听美柚，不使用屏幕坐标；广告或页面在 60 秒内始终未出现目标节点时会停止。由于美柚页面和奶量选择器的无障碍结构可能随版本更新，首次使用需要在安装美柚的真机上验证节点文案和滑动方向。
